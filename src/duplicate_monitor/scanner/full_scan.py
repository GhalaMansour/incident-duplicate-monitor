...
def _enrich_contractor(rows: list[dict], maximo_source=None) -> None:
    ...
    missing_contract = list({r["contract"] for r in rows
                             if r.get("contract") and not r.get("contractor")})
    if missing_contract:
        try:
    ...
    missing_og = list({r["ownergroup"] for r in rows
                       if r.get("ownergroup") and not r.get("contractor")})
    if missing_og:
        try:
...
def _enrich_reporter(rows: list[dict], maximo_source=None) -> None:
    ...
    if not maximo_source:
        return
    usernames = list({r["reporter"] for r in rows
                      if r.get("reporter") and not r.get("reporter_display")})
    if not usernames:
        return
    try:
...
def _seed_cache_from_disk() -> None:
    ...
    if disk_source in ("maximo", "file") and disk_count > _full_scan_row_count:
        ...
        log.info("Seeded _full_scan_row_count=%d from pkl (source=%s)",
                 _full_scan_row_count, disk_source)
    ...
    for r in rows:
        ...
        if sr:
            _rows_cache[sr] = r
    if _rows_cache:
        log.info("Seeded cache from disk (%d SRs, source=%s, _full_scan_row_count=%d)",
                 len(_rows_cache), disk_source, _full_scan_row_count)
        ...
        if gid:
            ...
            if gid not in _last_group_ids:
                ...
                new_groups.append({
                    ...
                    "tier":  g.get("tier", ""),
                    "size":  len(members),
                    "fault": (first.get("fault_full") or first.get("fault_orig") or first.get("fault") or "")[:60],
...
def _enrich_result(result: dict, sr_to_raw: dict) -> None:
    ...
    EXTRA = (
        "caller_name", "caller_phone", "caller_email", "caller_party",
        "reporter_phone", "reporter_email",
        "source", "source_desc",
        "ownergroup", "assigned_ownergroup",
        "lat", "lon", "priority_desc",
        "siteid", "history", "actstart", "actfinish", "statusdate",
        # Kidana custom — Contract / Contractor / الجهة / REQUESTOR NO. / SLA
        "contract", "contractor", "party", "requestor_no", "reported_name",
        "resp_time", "resp_esc", "targetstart", "targetfinish",
    )
...
def run_quick_scan(maximo_source) -> dict:
    ...
    try:
        ...
        changed_count = 0
        watch_fields = (
            "reported", "status", "status_desc", "location", "asset", "summary",
            "detail", "reporter", "workzone", "siteid", "priority",
            "priority_desc", "caller_phone", "caller_party", "source",
            "source_desc", "ownergroup", "assigned_ownergroup", "lat", "lon",
        )
        ...
        log.info("Quick scan: +%d new, %d changed SRs → %d total in cache",
                 len(new_srs), changed_count, len(all_rows))
        ...
        if len(all_rows) < _full_scan_row_count * 0.70:
            log.info(
                "Quick scan: skipping save — %d rows < %.0f%% of full-scan "
                "baseline %d",
                len(all_rows), 70.0, _full_scan_row_count,
            )
...
def run_scan(maximo_source, force: bool = False,
    ...
    try:
        ...
        log.info("Full scan: fetched %d rows (cutoff=%dh, max_days=%d)",
                 len(rows), cutoff_hours, md)
        ...
        log.info("Full scan: cache updated (%d rows, _full_scan_row_count=%d)",
                 len(_rows_cache), _full_scan_row_count)
