...
def _load_ref_auto(xls_path: Path, json_gz_path: Path,
...
_MAXIMO_MARKERS = {
    "Service Request", "Site", "Status", "Summary", "LOCATION",
    "Details", "Work Zone", "Asset", "History", "Source",
}
...
def _smart_text_compare(a: str, b: str) -> tuple[str, int, int]:
    ...
    التصنيف:
      ...
      'similar'       = قالب 80%+ بدون شرط الأرقام         → +1
    ...
    if final_pct >= 80:
        return ("similar", 3, final_pct)    # نص متشابه ≥80% → +3
    return ("different", 0, final_pct)


_DETAIL_STOPWORDS = {
    "من", "في", "عن", "على", "الى", "الي", "رقم", "عدد", "داخل", "بداخل",
    "اقرب", "معلم", "الدوره", "الدورة", "دوره", "دورة", "المياه", "مياه",
    "القسم", "النسائي", "حمامات", "حسب", "افادة", "المبلغ",
}
...
def _fuzzy_token_containment(a: set[str], b: set[str]) -> float:
    ...
    for tok in small:
        if tok in large:
            matched += 1
            continue
        if any(difflib.SequenceMatcher(None, tok, other, autojunk=False).ratio() >= 0.72
               for other in large):
            matched += 1
...
def _fmt_arabic_gap(td: timedelta) -> str:
    ...
    if total < 60: return f"+{total} ثانية"
    if total < 3600:
        m = total // 60
        return f"+{m} دقيقة"
    if total < 86400:
    ...
    days = total // 86400
    if days <= 14: return f"+{days} يوم"
    weeks = days // 7
    if weeks <= 8: return f"+{weeks} أسبوع"
    months = days // 30
    if months <= 12: return f"+{months} شهر"
    years = days // 365
    return f"+{years} سنة"
...
def _score(r1: dict, r2: dict) -> tuple[int, list[str], dict]:
    ...
    # ٤) نفس رقم المبلّغ +2
    if r1.get("requestor_no") and r2.get("requestor_no") and r1["requestor_no"] == r2["requestor_no"]:
        score += 2
...
def detect(df: pd.DataFrame, min_score: int = 5, max_days: int = 2) -> dict:
    ...
    def _try_pair(r1, r2):
        if r1["sr"] == r2["sr"]: return
        key = (min(r1["sr"], r2["sr"]), max(r1["sr"], r2["sr"]))
        if key in pair_keys: return
        pair_keys.add(key)
        ...
        # ── بوابة التفاصيل: النص يجب أن يكون متشابهاً 80% أو أكثر ──
        ...
        if not exact_fault_match:
            if txt_cls in ("different", "template_only") or tpl_pct <= 80:
                return
        ...
        pairs_raw.append({
            "r1": r1, "r2": r2,
            "score": sc, "reasons": reasons, "meta": meta,
        })
    ...
    for root, members in clusters_map.items():
        ...
        for a, b in itertools.combinations(members, 2):
            key = (min(a,b), max(a,b))
            if key in pair_scores: inner_pairs.append(pair_scores[key])
        ...
        for p in inner_pairs:
            for r in p["reasons"]:
                if r not in seen_reasons:
                    all_reasons.append(r); seen_reasons.add(r)
...
GROUP_COLS = [
    ...
    "Status", "Status Date", "Status Description",
    "Summary", "Work Zone", "LOCATION", "وصف المعلم", "المربع", "المنطقة",
    "Asset", "وصف الأصل", "REQUESTOR NO.", "Contract", "Contractor",
    "REPORTED NAME", "Reported By",
    "زمن الاستجابه", "Response Esclation", "Resolution Time", "Resolution Escalation",
    "الجهة", "Details",
...
def write_report(detection: dict, output_path: str, total_rows: int):
    ...
    def _hrow(ws, row, vals, bg=C_DARK, fg="FFFFFF"):
        for ci, v in enumerate(vals, 1):
            ...
            c.fill = _fill(bg); c.font = _font(True, 10, fg)
            c.alignment = _al("center"); c.border = _thin()
        ws.row_dimensions[row].height = 24
    def _cell(ws, row, col, val, bg=C_WHITE, bold=False, wrap=False,
              align="right", color="000000", size=10):
        c = ws.cell(row=row, column=col, value=val)
        c.fill = _fill(bg); c.font = _font(bold, size, color)
        c.alignment = _al(align, wrap=wrap); c.border = _thin()
        return c
    ...
    t.fill = _fill(C_DARK); t.font = _font(True, 13, C_GOLD)
    t.alignment = _al("center"); ws1.row_dimensions[1].height = 30
    ...
    for gi, g in enumerate(groups, 1):
        ...
        for mi, row in enumerate(members, 1):
            ...
            if mi == 1 or prev_dt is None or cur_dt is None:
                gap_str = "—"; gap_color = C_WHITE
            else:
                gap_td = cur_dt - prev_dt
                gap_str = _fmt_arabic_gap(gap_td)
                # لون الفارق
                days = gap_td.days
                if days == 0:    gap_color = DATE_COLORS["same_day"]
                elif days <= 7:  gap_color = DATE_COLORS["same_week"]
                else:            gap_color = DATE_COLORS["far"]
            ...
            _cell(ws1, ri, 2,  f"{mi}" + (" (الأصل)" if is_origin else ""),
                  bg=bg, bold=is_origin, align="center")
            _cell(ws1, ri, 3,  gap_str, bg=gap_color, bold=True, align="center")
            _cell(ws1, ri, 4,  " | ".join(g["reasons"]) if is_origin else "",
                  bg=bg, wrap=True, color=tier_fg)
            ...
            _cell(ws1, ri, 35, _fmt_date(cur_dt) if cur_dt else row["reported"],
                  bg=bg, align="center")
    ...
    col_widths = {
        1:9,  2:11, 3:16, 4:40,                 # أعمدة المجموعة
        5:14, 6:8,  7:18, 8:12, 9:12, 10:18, 11:22,  # SR…Status Description
        12:32,13:14,14:14,15:28,16:12,17:12,    # Summary…المنطقة (15=وصف المعلم)
        18:14,19:28,20:16,21:14,22:20,          # Asset…Contractor (19=وصف الأصل)
        23:20,24:20,                             # REPORTED NAME, Reported By
        25:14,26:14,27:14,28:14,29:14,          # زمن الاستجابه…الجهة
        30:45,                                  # Details
        31:14,32:14,33:14,34:18,                # Lat, Lon, Priority, Ticket
        35:18,36:18,37:18,                      # تواريخ
    }
    ...
    SUMMARY_COLS = [
        "رقم المجموعة", "حجم المجموعة", "الثقة", "النقاط",
        "أعضاء المجموعة", "تاريخ أول بلاغ", "تاريخ آخر بلاغ",
        "المدى الزمني", "أسباب الكشف",
    ]
    ...
    t2.fill = _fill(C_DARK); t2.font = _font(True, 13, C_GOLD)
    t2.alignment = _al("center"); ws2.row_dimensions[1].height = 28
    ...
    for gi, g in enumerate(groups, 1):
        ...
        _cell(ws2, ri, 8, _fmt_arabic_gap(ts.get("span")) if ts.get("span") else "—", bg=bg, align="center")
    ...
    th.fill = _fill(C_DARK); th.font = _font(True, 13, C_GOLD)
    th.alignment = _al("center"); ws3.row_dimensions[1].height = 32
    ...
    for ri2, item in enumerate(stats, 2):
        ...
        ca.fill = _fill(bg); ca.font = _font(True); ca.alignment = _al(); ca.border = _thin()
        cb = ws3.cell(ri2, 2, val)
        cb.fill = _fill(bg); cb.font = _font(True, 12, C_GOLD)
        cb.alignment = _al("center"); cb.border = _thin()
...
def main():
    ...
    p.add_argument("--min-score", type=int, default=5,
                   help="الحد الأدنى للنقاط (افتراضي: 5 = محتمل+مؤكد)")
    p.add_argument("--max-days", type=int, default=2,
    ...
    safe_all_rows = [
        {k: v for k, v in r.items() if k != "_row_orig"}
        for r in detection["all_rows"]
    ]
    serializable = {
        "groups": [{
            "members":     g["members"],
